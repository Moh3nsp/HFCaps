...
class GlobalFedCaps(nn.Module):
    def __init__(self,
        ...
        self.conv_1 = nn.ModuleList([nn.Conv2d(
            1, conv1_Channel[i], kernel_size=kernel_size[i], stride=1) for i in range(ensimble_num)])

        self.primaryCaps = nn.ModuleList([PrimaryCapsLayer(
            conv1_Channel[i], primary_channel[i], capsule_dimention[i], kernel_size=primaryCaps_filter_size[i], stride=2) for i in range(ensimble_num)])
        local_models = [LocalFedCapsNet(
            conv1_Channel[i], primary_channel[i], capsule_dimention[i], kernel_size[i], primaryCaps_filter_size[i]) for i in range(ensimble_num)]
        # ensimble_num * primary_channel * number_primaryCaps * number_primaryCaps
        self.num_primaryCaps = torch.sum(torch.tensor([local_models[i].caps_number for i in range(ensimble_num)]))

        self.routing_module = AgreementRouting(
            self.num_primaryCaps, n_classes, routing_iterations)
        self.digitCaps = CapsLayer(
            self.num_primaryCaps, capsule_dimention[0], n_classes, 16, self.routing_module)
